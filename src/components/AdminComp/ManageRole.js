...
const ManageRole = ({value, setValue}) => {
    ...
    const isRoleButtonModelSekected = (typeId) => {
        setModalShow(true)
          ...
          <div className='mr-bg'> 
            <div className='mr-centered'> 
              {userRoleModalData.map((type)=>(
                 <div index={type.id}>
                 <Button variant="info" className='mr-color' onClick={() => isRoleButtonModelSekected(type.id)}>
                  {type.designation}
                 </Button> 
                </div> 
            ...
            <UserRoleAssigmentModal show={modalShow} users={users} onHide={()=> setModalShow(false)} typeSelected={typeSelected} />
